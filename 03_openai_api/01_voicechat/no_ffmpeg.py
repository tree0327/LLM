...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.caption(
            "🎙️ 강의실 PC 일부에서 ffmpeg(Device Guard) 차단 이슈가 있어, 녹음 위젯 대신 업로드/텍스트 입력으로 구성했습니다."
        )
    ...
    with col1:
        ...
        typed_text = st.text_input(
            "질문을 입력하세요", value="", placeholder="예) 점심 뭐 먹을까?"
        )

        # 실행 버튼 (업로드 또는 텍스트 둘 중 하나라도 있으면 활성)
        can_run = (uploaded is not None and stt_file is not None) or (
            typed_text.strip() != ""
        )
    ...
    with col2:
        ...
        if stt_file is None:
            st.info(
                "ℹ️ openai_service.py에 stt_file()이 없어서, 현재는 '텍스트 질문'만 가능합니다. (아래 안내 참고)"
            )
